...
const approveWithdrawal = async (requestId, approvalNote, image) => {
  try {
    ...
    // Upload the image (for this example, it's a local upload)
    // You can replace this with AWS S3 or another file storage service if needed
    ...
    // Add the image URL and note to the withdrawal request
    request.status = 'approved';
    request.approvalNote = approvalNote; // Add note to the request
    request.image = image; // Add image URL to the request
...
const getMyWithdrawalRequests  = async (filter, options) => {
  ...
  for (const key of Object.keys(filter)) {
    if (filter[key] !== "") {
      if (key === "status") {
        // Use regex for partial matching and case-insensitive search for status
        query[key] = { $regex: new RegExp(filter[key], 'i') };
      } else {
        // For other fields, use the direct filter value
        query[key] = filter[key];
